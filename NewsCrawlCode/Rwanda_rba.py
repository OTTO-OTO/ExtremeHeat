...
class AirSpiderDemo(feapder.AirSpider):
    ...
    sql = """ select db_name from keywords where country='Rwanda' and language = '英语'"""
    ...
    def start_requests(self):
        for keyword in self.keywords:
            page = 0
            url = "https://www.rba.co.rw/search"
            params = {
                "search_q": f"{keyword}",
                "page": "0"
            }
    ...
    def download_midware(self, request):
        request.headers = {
            ...
            "priority": "u=0, i",
            "referer": "https://www.rba.co.rw/search?search_q=heavy+rain",
    ...
    def parse_detail(self, request, response):
        ...
        items.content = "".join(response.xpath("//div[@class='article_p article_style']//p/text()").extract())
        items.author = ''
        items.pubtime = response.xpath("//span[@class='old']/text()").extract_first()
        print(items)
        if items.content:
            yield items
