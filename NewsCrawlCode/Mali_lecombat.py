...
class AirSpiderDemo(feapder.AirSpider):
    ...
    sql = """ select db_name from keywords where country='Mali' and language = '法语'"""
    ...
    def download_midware(self, request):
        request.headers = {
            ...
            "priority": "u=0, i",
            "referer": "https://lecombat.fr/?s=Temp%C3%A9rature+%C3%A9lev%C3%A9e",
    ...
    def parse_url(self, request, response):
        ...
        url = f"https://lecombat.fr/page/{current_page}/"
        params = {
            "s": f"{current_keyword}"
        }
    ...
    def parse_detail(self, request, response):
        items = request.items
        items.title = response.xpath("//span[@itemprop='name']/text()").extract_first()
        items.content = "".join(
            response.xpath("//div[@class='entry']//p/text()").extract())
        items.author = response.xpath("//span[@class='post-meta-author']/a/text()").extract_first()
        items.pubtime = response.xpath("//span[@class='tie-date']/text()").extract_first()
        print(items)
        if items.content:
            yield items
