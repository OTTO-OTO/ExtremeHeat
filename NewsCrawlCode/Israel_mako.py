...
class AirSpiderDemo(feapder.AirSpider):
    ...
    sql = """ select db_name from keywords where country='Israel' and language = '希伯来语'"""
    ...
    print(f"{table}创建成功<=================")
    keywords = db.find(f"select keywords_list from keywords where language = '希伯来语' and country='{table}'", to_json=True)[0].get(
        "keywords_list")
    ...
    def start_requests(self):
        for keyword in self.keywords:
            page = 1
            url = "https://www.mako.co.il/AjaxPage"
            params = {
                "jspName": "search_results_main_response.jsp",
                "searchstring_input": f"{keyword}",
                "page": "1",
                "tab": "search_results_tab_general",
                "searchString": "%D7%98%D7%9E%D7%A4%D7%A8%D7%98%D7%95%D7%A8%D7%95%D7%AA+%D7%92%D7%91%D7%95%D7%94%D7%95%D7%AA",
                "channelId": "3d385dd2dd5d4110VgnVCM100000290c10acRCRD",
                "vgnextoid": "3d385dd2dd5d4110VgnVCM100000290c10acRCRD",
                "formType": "regular"
            }
    ...
    def download_midware(self, request):
        request.headers = {
            ...
            "priority": "u=0, i",
            "referer": "https://www.newsit.gr/?s=%CE%A5%CF%88%CE%B7%CE%BB%CE%AD%CF%82+%CE%B8%CE%B5%CF%81%CE%BC%CE%BF%CE%BA%CF%81%CE%B1%CF%83%CE%AF%CE%B5%CF%82&sort=title",
    ...
    def parse_url(self, request, response):
        ...
        for item in links:
            ...
            items.article_url ="https://www.mako.co.il" + item.get("pageURL")
            # items.title = item.get("post_title")
            items.country = self.country
            items.keyword = request.keyword  # 确保 items.keyword 被正确赋值
            items.pubtime = item.get("date")
        ...
        params = {
            "jspName": "search_results_main_response.jsp",
            "searchstring_input": f"{current_keyword}",
            "page": f"{current_page}",
            "tab": "search_results_tab_general",
            "searchString": "%D7%98%D7%9E%D7%A4%D7%A8%D7%98%D7%95%D7%A8%D7%95%D7%AA+%D7%92%D7%91%D7%95%D7%94%D7%95%D7%AA",
            "channelId": "3d385dd2dd5d4110VgnVCM100000290c10acRCRD",
    ...
    def parse_detail(self, request, response):
        ...
        items.content = "".join(response.xpath("//section[@class='article-body']//p/text()").extract())
        items.author = ''
        # items.pubtime = response.xpath("//section[@class='article-body']//p/text()").extract_first()
        print(items)
        if items.content:
            yield items
