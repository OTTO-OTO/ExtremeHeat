...
class AirSpiderDemo(feapder.AirSpider):
    ...
    def parse_url(self, request, response):
        ...
        try:
            ...
            links = json.loads(response.text.split("searchPage.resultcallback(")[-1].split(");")[0])['items']
        ...
        for item in links:
            ...
            items.article_url = "https://www.ctvnews.ca" + item.get("link")
            items.title = item.get("title")
            items.country = self.country
            items.pubtime = item.get("pubdateunix")
    ...
    def parse_detail(self, request, response):
        try:
            ...
            # 方法1: 从 Fusion.globalContent 中提取
            pattern = r'Fusion\.globalContent=(.*?);Fusion\.spa=false'
