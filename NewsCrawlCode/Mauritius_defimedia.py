...
class AirSpiderDemo(feapder.AirSpider):
    ...
    def start_requests(self):
        for keyword in self.keywords:
            page = 0
            url = "https://defimedia.info/recherche-articles"
            params = {
                "combine": f"{keyword}",
                "page": "0"
            }
    ...
    def parse_detail(self, request, response):
        ...
        items.content = "".join(
            response.xpath(
                "//div[@class='field field--name-body field--type-text-with-summary field--label-hidden field--item']//text()").extract())
        items.author = ""
        items.pubtime = response.xpath("//div[@class='submitted']/text()").extract_first()
        print(items)
        if items.content:
            yield items
