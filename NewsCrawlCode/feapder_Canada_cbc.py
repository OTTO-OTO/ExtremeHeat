...
class AirSpiderDemo(feapder.AirSpider):
    ...
    def parse_url(self, request, response):
        ...
        try:
            ...
            # 保存响应内容到文件以便调试
            if request.page == 1:
                with open(f"cbc_{current_keyword}_response.html", "w", encoding="utf-8") as f:
                    f.write(response.text)
                print(f"已保存响应内容到 cbc_{current_keyword}_response.html")
            ...
            for link in links:
                ...
                if link.startswith('http'):
                    if 'cbc.ca' in link and '/news/' in link:
                        valid_links.append(link)
                elif link.startswith('/'):
                    full_url = 'https://www.cbc.ca' + link
                    if '/news/' in full_url:
