...
class AirSpiderDemo(feapder.AirSpider):
    ...
    sql = """ select db_name from keywords where country='Iceland' and language = '冰岛语'"""
    ...
    print(f"{table}创建成功<=================")
    keywords = db.find(f"select keywords_list from keywords where language = '冰岛语' and country='{table}'", to_json=True)[0].get(
        "keywords_list")
