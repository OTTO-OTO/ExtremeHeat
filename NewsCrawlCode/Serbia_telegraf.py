...
class AirSpiderDemo(feapder.AirSpider):
    ...
    sql = """ select db_name from keywords where country='Serbia' and language = '意大利语'"""
    ...
    print(f"{table}创建成功<=================")
    keywords = db.find(f"select keywords_list from keywords where language = '意大利语' and country='{table}'", to_json=True)[0].get(
        "keywords_list")
    print("待抓取的关键词列===========>", keywords)
    def start_requests(self):
        for keyword in self.keywords:
            page = 1
            url = "https://www.telegraf.rs/pretraga/page/1"
            params = {
                "q": f"{keyword}"
            }
    ...
    def download_midware(self, request):
        request.headers = {
            ...
            "priority": "u=0, i",
            "referer": "https://www.telegraf.rs/pretraga/?q=visoka%20temperatura",
    ...
    def parse_url(self, request, response):
        ...
        links = response.xpath("//div[@class='filter-page-content-inner']//figcaption/a/@href").extract()
        ...
        url = f"https://www.telegraf.rs/pretraga/page/{current_page}"
        params = {
            "q": f"{current_keyword}"
        }
    ...
    def parse_detail(self, request, response):
        ...
        items.content = "".join(
            response.xpath("//div[@class='article-content']//p//text()").extract())
        items.author = response.xpath("//meta[@property='article:author']/@content").extract_first()
