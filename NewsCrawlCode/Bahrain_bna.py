...
class AirSpiderDemo(feapder.AirSpider):
    ...
    sql = """ select db_name from keywords where country='Bahrain' and language='阿拉伯语'"""
    ...
    def start_requests(self):
        for keyword in self.keywords:
            page = 1
            url = "https://www.bna.bh/bnaWebService.aspx/fnGetWebsiteSearchNew"
            data = {
                "RowNumber": 0,
                "NewsKeyword": f"{keyword}",
                "RowNumberArchive": 0,
                "pageIndex": 1,
                "pagesize": "30"
            }
    ...
    def download_midware(self, request):
        request.headers = {
            ...
            "origin": "https://www.bna.bh",
            "priority": "u=0, i",
            "referer": "https://www.bna.bh/search.aspx?search-input=%d9%81%d9%8a%d8%b6%d8%a7%d9%86",
    ...
    def parse_url(self, request, response):
        ...
        cleaned_text = re.sub(r'\\u[0-9a-fA-F]{4}/', '', response.text)
        print(cleaned_text)
        pattern = r'\.aspx\?cms=[^"\\]+'
        links = re.findall(pattern, cleaned_text)
        ...
        for item in links:
            ...
            items.article_url = "https://www.bna.bh/"+item
    ...
    def parse_detail(self, request, response):
        ...
        items.author = ''
        items.pubtime = response.xpath("(//dd[@class='createdby']/text())[last()]").extract_first()
        print(items)
        if items.content:
            yield items
