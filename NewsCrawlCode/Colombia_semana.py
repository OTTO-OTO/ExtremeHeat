...
class AirSpiderDemo(feapder.AirSpider):
    ...
    def parse_url(self, request, response):
        ...
        for item in links:
            ...
            items.article_url ="https://www.semana.com" +item
    ...
    def parse_detail(self, request, response):
        items = request.items
        items.table_name = self.table
        items.title = response.xpath("//meta[@property='title']/@content").extract_first()
        items.content = "".join(response.xpath(
            "//p[@data-type='text']//text()").extract()).strip()
        items.author = ''
