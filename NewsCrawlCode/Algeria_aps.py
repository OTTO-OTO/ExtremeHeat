...
class AirSpiderDemo(feapder.AirSpider):
    ...
    def start_requests(self):
        for keyword in self.keywords:
            page = 0
            url = "https://www.aps.dz/recherche"
            params = {
                "searchword": f"{keyword}",
                "ordering": "newest",
                "searchphrase": "all",
                "limit": "20",
                "start": "0"
    ...
    def parse_url(self, request, response):
        ...
        links = response.xpath("//dl[@class='search-results']/dt/a/@href").extract()
    ...
    def parse_detail(self, request, response):
        ...
        items.content = "".join(response.xpath("//div[@class='itemFullText']//p/text()").extract()).replace('\n','').replace('\r','')
        items.author = ''
        items.pubtime = response.xpath("(//span[@class='itemDateCreated']/text())[last()]").extract_first()
        print(items)
        if items.content:
            yield items
