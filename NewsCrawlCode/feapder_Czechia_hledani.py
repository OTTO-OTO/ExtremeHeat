...
class AirSpiderDemo(feapder.AirSpider):
    ...
    country = 'Czechia'
    table = 'Czechia_hledani'
    keywords = [
    "teplota", "extrémní vlna", "vysoká teplota", "extrémní teplota", "vlna tepla", "nárůst teploty", "vlivy tepla", "teplota", "silné teploty", "stupňování teploty", "teplotní událost", "stupňování teploty", "silné srážky", "silné deště", "silné deště", "extrémní srážky", "sucha", "těžká sucha", "dlouhodobá sucha", "vodu schodnost", "výpadky elektřiny", "teplotní výpadky", "výpadky elektřiny", "výpadky elektřiny", "požár", "teplotní požár", "teplotní požár", "teplotní požár", "teplotní požár", "vlivy na zemědělství", "vlna tepla", "poškození plodin", "zemědělský tepelný stres", "hypoxie", "otročení", "teplotní otročení", "teplotní otročení", "dopravní dopady", "teplotní dopady", "vlna tepla", "teplotní dopady", "ekologická katastrofa", "teplotní katastrofa", "teplotní prostředí", "vlivy tepla na biodiverzitu", "vlna tepla", "znečištění", "teplotní znečištění", "teplotní znečištění", "korálová bílost", "teplotní korálové útesy", "teplotní bílost korálových útesů"
]

    def start_requests(self):
        for keyword in self.keywords:
            page = 0
            url = "https://hledani.rozhlas.cz/hledani-api/search"
            params = {
                "page%5Blimit%5D": "10",
                "filter%5Bfulltext%5D": f"{keyword}",
                "page%5Boffset%5D": "10",
                "sort": "since"
            }
    ...
    def parse_url(self, request, response):
        ...
        for item in links:
            ...
            items.article_url = item.get("_source").get("url")
        ...
        url = "https://hledani.rozhlas.cz/hledani-api/search"
        params = {
            "page%5Blimit%5D": f"{current_page * 10}",
            "filter%5Bfulltext%5D": f"{current_keyword}",
            "page%5Boffset%5D": "10",
            "sort": "since"
        }
    ...
    def parse_detail(self, request, response):
        ...
        items.content = "".join(response.xpath("//div[@class='b-detail']//p/text()").extract()) if response.xpath("//div[@class='b-detail']//p/text()").extract() else "".join(response.xpath("//div[@class='content-container']//p/text()").extract())
        items.author = ''
        items.pubtime = ''
        print(items)
        if items.content:
