...
class AirSpiderDemo(feapder.AirSpider):
    ...
    def start_requests(self):
        for keyword in self.keywords:
            page = 1
            url = "https://www.newtimes.co.rw/search"
            params = {
                "query": f"{keyword}",
                "pgno": "1"
            }
    ...
    def parse_detail(self, request, response):
        ...
        pattern = r'"article_body"\s*:\s*"((?:[^"\\]|\\.)*)"'
        match = re.search(pattern, response.text, re.DOTALL).group(1)
        items.content = "".join(etree.HTML(match).xpath("//p//text()"))
        items.author = ''
