...
class AirSpiderDemo(feapder.AirSpider):
    ...
    sql = """ select db_name from keywords where country='Bahamas' and language='英语'"""
    ...
    def download_midware(self, request):
        request.headers = {
            ...
            "Referer": "https://www.tribune242.com/search/vertical/news.story/?q=heavy+rain",
    ...
    def parse_detail(self, request, response):
        ...
        items.content = "".join(response.xpath("//div[@class='story_body ']//p/text()").extract())
        items.author = ''
        items.pubtime = response.xpath("//p[@class='datetime']/a/text()").extract_first()
        print(items)
        if items.content:
            yield items
