...
class AirSpiderDemo(feapder.AirSpider):
    ...
    def __init__(self):
        super().__init__()
        # 使用字典存储每个关键词的上一页链接，避免不同关键词之间的干扰
        self.previous_links = {}
    
    def start_requests(self):
        for keyword in self.keywords:
            page = 1
            url = "https://www.independent.co.uk/topic/{}".format(keyword.lower())
    ...
    def parse_url(self, request, response):
        ...
        # 提取新闻链接
        links = response.xpath("//a[@class='sc-oq6ovx-0 bWGYNn title']/@href").extract()
        # 去重
        ...
        self.previous_links[current_keyword] = current_links  # 更新当前关键词的上一页链接列表
        ...
        for item in links:
            # 构建完整的URL
            if item.startswith('/'):
                full_url = "https://www.independent.co.uk" + item
            else:
                full_url = item
                
            items = Item()
            items.table_name = self.table
            items.article_url = full_url
            items.country = self.country
            items.keyword = current_keyword
            yield feapder.Request(url=full_url, callback=self.parse_detail, items=items)

        # 翻页 - 首先尝试使用Google搜索样式的分页控件
        next_page_element = response.xpath("//div[@class='gsc-cursor']//div[@class='gsc-cursor-page' and @aria-label='Page {0}']".format(request.page + 1))
        if next_page_element:
            # 如果找到Google搜索样式的分页控件，使用JavaScript点击的方式
            # 但由于我们使用的是feapder，这里还是通过URL参数来翻页
            current_page = request.page + 1
            url = "https://www.independent.co.uk/topic/{0}?page={1}".format(current_keyword.lower(), current_page)
            yield feapder.Request(url, callback=self.parse_url, page=current_page, keyword=current_keyword, filter_repeat=True)
        else:
            # 尝试另一种分页方式 - 检查是否有下一页按钮
            next_page_link = response.xpath("//a[contains(text(), 'Next') or contains(@class, 'next')]/@href").extract_first()
            if next_page_link:
                if next_page_link.startswith('/'):
                    next_page_url = "https://www.independent.co.uk" + next_page_link
                else:
                    next_page_url = next_page_link
                current_page = request.page + 1
                yield feapder.Request(next_page_url, callback=self.parse_url, page=current_page, keyword=current_keyword, filter_repeat=True)
            else:
                # 如果没有找到分页控件，继续使用原来的URL参数方式
                current_page = request.page + 1
                url = "https://www.independent.co.uk/topic/{0}?page={1}".format(current_keyword.lower(), current_page)
                yield feapder.Request(url, callback=self.parse_url, page=current_page, keyword=current_keyword, filter_repeat=True)
    ...
    def parse_detail(self, request, response):
        ...
        # 提取文章内容
        content_elements = response.xpath("//div[@class='sc-153d5819-0 gVqCzD']//p/text()").extract()
