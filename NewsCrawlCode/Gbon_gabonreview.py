...
class AirSpiderDemo(feapder.AirSpider):
    ...
    sql = """ select db_name from keywords where country='Gabon' and language = '法语'"""
    ...
    def start_requests(self):
        for keyword in self.keywords:
            page = 1
            url = f"https://www.gabonreview.com/page/1/"
            params = {
                "s": f"{keyword}",
                "x": "0",
                "y": "0"
    ...
    def parse_url(self, request, response):
        ...
        for item in links:
            ...
            yield feapder.Request(url=items.article_url, callback=self.parse_detail, items=items, allow_redirects=False)

        current_page = request.page + 1
        url = f"https://www.gabonreview.com/page/{current_page}/"
        params = {
            "s": f"{current_keyword}",
            "x": "0",
            "y": "0"
    ...
    def parse_detail(self, request, response):
        ...
        items.content = "".join(response.xpath("//div[@id='main']//p//text()").extract())
        items.author = ''
