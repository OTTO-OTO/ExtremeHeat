...
class AirSpiderDemo(feapder.AirSpider):
    ...
    sql = """ select db_name from keywords where country='Hungary'"""
    ...
    print(f"{table}创建成功<=================")
    keywords = \
        db.find(f"select keywords_list from keywords where language = '匈牙利语' and country='{table}'", to_json=True)[
            0].get(
            "keywords_list")
    ...
    def start_requests(self):
        for keyword in self.keywords:
            page = 0
            url = "https://nepszava.hu/_includes/cikk/api/search.php"
            params = {
                "ajax": "true",
                "s": f"{keyword}",
                "dstart": "2000.01.01",
    ...
    def parse_url(self, request, response):
        ...
        for key,item in links.items():
            ...
            items.article_url ="https://nepszava.hu/" + item[0].get("link")
