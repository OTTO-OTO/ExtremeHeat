...
class AirSpiderDemo(feapder.AirSpider):
    ...
    sql = """ select db_name from keywords where country='China' and language='中文'"""
    mysql_db = db.find(sql, to_json=True)[0].get("db_name")
    print("待写入的数据库是:==================>", mysql_db)
    # 判断数据库是否存在
    db.execute(f"CREATE DATABASE IF NOT EXISTS `{mysql_db}` CHARACTER SET utf8mb4")
    ...
    print(f"{table}创建成功<=================")
    keywords = db.find(f"select keywords_list from keywords where language = '中文' and country='{table}'", to_json=True)[0].get("keywords_list")
    ...
    def parse_detail(self, request, response):
        ...
        items.content = "".join(response.xpath(
            "//div[contains(@class, 'cnt_bd')]//p/text() | //div[@class='content_area']//p/text() | //div[@class='body']//p/text()").extract()).replace("\r",'').replace("\t",'').replace("\n",'')  # //div[@class='body']//p/text()  //div[@class='cnt_bd']//p/text() //div[@class='cnt_bd']//p/text()
        items.author = ''
        pattern = r'\bvar\s+publishDate\s*=\s*"\s*(\d{14})\s*"\s*;'
        pubtime = ""
        match = re.search(pattern, response.text)
        if match:
            print("Matched value:", match.group(1))
            print(type(match.group(1)))
            pubtime = datetime.strptime(match.group(1), "%Y%m%d%H%M%S").strftime("%Y-%m-%d %H:%M:%S")
        else:
            print("No match found.")
        items.pubtime = pubtime
        print(items)
        # if items.content:
        #     yield items
        #
