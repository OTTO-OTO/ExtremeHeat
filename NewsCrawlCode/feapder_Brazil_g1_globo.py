...
class AirSpiderDemo(feapder.AirSpider):
    ...
    def start_requests(self):
        for keyword in self.keywords:
            ...
            params = {
                ...
                "order": "recent",
                "species": "notícias",
                "from": "2000-01-01T00:00:00+0800",
                "to": "2025-02-26T23:59:59+0800",
                "page": "1"
            }
            yield feapder.Request(url, params=params, callback=self.parse_url, page=page, keyword=keyword,
                                  allow_redirects=False, filter_repeat=True)
    ...
    def parse_url(self, request, response):
        ...
        try:
            ...
            data = response.xpath("//a[@class='widget--info__media widget--info__media--video']/@href").extract()
            print(data)
    ...
    def parse_detail(self, request, response):
        try:
            resp = response.text.split("replace(")[-1].split(");")[0]
            print(resp)
            resp_data = curl_requests.get(resp).text
            html = etree.HTML(resp_data)
            items = request.items
            items.table_name = self.table
            
            # 尝试提取标题
            title = html.xpath("//h1/text()")[0] if html.xpath("//h1/text()") else None
            if title:
                # 清理HTML标签
            ...
            # 尝试提取内容，使用多个选择器
            content_selectors = [
                "//article[@itemprop='articleBody']//div//p/text()",
                "//article//p/text()",
            ...
            for selector in content_selectors:
                content_parts = html.xpath(selector)
                if content_parts:
            ...
            if not pubtime:
                pubtime = html.xpath("//time/@datetime")[0] if html.xpath("//time/@datetime") else None
