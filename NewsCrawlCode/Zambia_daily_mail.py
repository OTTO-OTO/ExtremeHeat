...
class AirSpiderDemo(feapder.AirSpider):
    ...
    sql = """ select db_name from keywords where country='Zambia' and language='英语'"""
    ...
    def start_requests(self):
        for keyword in self.keywords:
            page = 1
            url = "https://www.daily-mail.co.zm/page/1/"
            params = {
                "s": f"{keyword}"
            }
    ...
    def download_midware(self, request):
        request.headers = {
            ...
            "Referer": "https://www.daily-mail.co.zm/?s=rain",
    ...
    def parse_url(self, request, response):
        ...
        url = f"https://www.daily-mail.co.zm/page/{current_page}/"
        params = {
            "s": f"{current_keyword}"
        }
    ...
    def parse_detail(self, request, response):
        ...
        items.author = ''
        items.pubtime = response.xpath("//span[@class='elementor-icon-list-text elementor-post-info__item elementor-post-info__item--type-date']/text()").extract_first()
        print(items)
        if items.content:
            yield items
