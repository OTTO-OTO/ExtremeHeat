function [mclim,m90]=mhw_detect(temp,time,cli_start,cli_end,varargin)
...
%       - mhw_end - end date of each event. "yyyymmdd", e.g., 19860704
%       - mhw_dur - duration of each event. units: days
...
%[vEvent, vThreshold,vWindowHalfWidth,vsmoothPercentileWidth,vminDuration,vmaxGap,ClimTemp,ClimTime,vpercentile]...
%    = internal.stats.parseArgs(paramNames, defaults);
...
date_true0=datevec(cli_start-vWindowHalfWidth:cli_end+vWindowHalfWidth);
date_true=date_true0(:,1:3);
...
%mclim=NaN(size(temp,1),size(temp,2),366); m90=NaN(size(temp,1),size(temp,2),366);
mclim=nan(jm,im,366); m90=nan(jm,im,366);
tic;
for i=1:366
    if mod(i,20)==0; toc; disp([num2str(i),'/',num2str(366)]); tic; end
    if i == 60
    else
        ind_fake=ind;
        ind_fake(fake_doy==i & ~ismember(datenum(date_true0),cli_start:cli_end))=nan;
        ind0=any(ind_fake'>=(ind_fake(fake_doy == i)-vWindowHalfWidth) & ind_fake' <= (ind_fake(fake_doy ==i)+vWindowHalfWidth),2); 
        switch vpercentile
        case 'matlab'
            m90(:,:,i) = quantile(temp_clim(:,:,ind0),vThreshold,3);
            mclim(:,:,i) = mean(temp_clim(:,:,ind0),3,'omitnan');
        ...
        case 'python'
            ...
            data_thre=num2cell(temp_clim(:,:,ind0),3);
