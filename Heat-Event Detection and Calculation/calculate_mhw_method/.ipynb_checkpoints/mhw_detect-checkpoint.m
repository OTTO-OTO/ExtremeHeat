function [MHW,mclim,m90,mhw_ts]=mhw_detect(temp,time,m90,mclim,mhw_start,mhw_end,varargin)
...
MHW=[];
num=0; mod_num=10;
%% Detecting size of MHW/MCS
tic;
switch vEvent
    case 'MHW'
        for i=1:x_size
            if mod(i,mod_num)==0; toc; disp(['Detecting size of MHW/MCS ',num2str(i),'/',num2str(x_size)]); tic; end
            for j=1:y_size
                ...
                if sum(isnan(squeeze(mbigadd(i,j,:))))~=size(mbigadd,3)
                    ...
                    if ~isempty(potential_event)
                        ...
                        if ~isempty(potential_event)
                            ...
                            num=num+size(potential_event,1); 
                        end
                    end
                end
            end
...
switch vEvent
    case 'MHW'
        ...
        for i=1:x_size
            if mod(i,mod_num)==0; toc; disp(['Detecting MHW/MCS ',num2str(i),'/',num2str(x_size)]); tic; end
            for j=1:y_size
                ...
                if sum(isnan(squeeze(mbigadd(i,j,:))))~=size(mbigadd,3)
                    ...
                    if ~isempty(potential_event)
                        ...
                        if ~isempty(potential_event)
                            ...
                            ind=num+1:num+size(potential_event,1);
                            MHW(ind,:)=[mhwstart mhwend mduration mhwint_max mhwint_mean mhwint_var mhwint_cum repmat(i,size(mhwstart,1),1) repmat(j,size(mhwstart,1),1)];
                            num=num+size(potential_event,1);
                        end
                    end
                end
